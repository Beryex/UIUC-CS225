...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    if(elements.back() < val) return elements.size();

    // find the target using binary search
    size_t left_bound = 0, right_bound = elements.size() - 1;
    while(left_bound <= right_bound){
        size_t media = (left_bound + right_bound) / 2;
        if(elements[media] == val) return media;
        if(elements[media] > val){
            right_bound = media - 1;
        } else {
            left_bound = media + 1;
        }
