...
void LPHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    size_t ans = idx;
    if(!table[ans]){
        should_probe[ans] = true;   
        table[ans] = new pair<K, V>(key, value);
        return;
    }
    while(table[ans]) {
        ans = (idx + offset * offset) % size;
        if(!table[ans]){
            should_probe[ans] = true;   
            table[ans] = new pair<K, V>(key, value);
            return;
...
int LPHashTable<K, V>::findIndex(const K& key) const
{
    ...
    size_t offset = 1;
    size_t ans = idx;
    while (should_probe[ans]) {
        if (table[ans] != NULL && table[ans]->first == key)
            return ans;
        ans = (idx + offset * offset) % size;
        if (table[ans] != NULL && table[ans]->first == key)
            return ans;
...
void LPHashTable<K, V>::resizeTable()
{
    ...
    for (size_t i = 0; i < size; i++) {
        if (table[i] != NULL) {
            ...
            size_t offset = 1;
            while (temp[ans] != NULL){
                ans = (idx + offset * offset) % newSize;
                if(temp[ans] == NULL) break;
                ans = (idx - offset * offset) % newSize;
                offset++;
            }
            temp[ans] = table[i];
