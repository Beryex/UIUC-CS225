...
namespace cs225{
    // default constructor
    Image::Image() : PNG() {
        // nothing here, see initialization list
    }
    ...
    void Image::scale(double factor){
        ...
        delete old;
        old = nullptr;
        // my original scale has some problems
        /*
    ...
    void Image::rotate(int degree){
        ...
        if(degree == 180){
            unsigned new_h = height();
            unsigned new_w = width();
            Image result;
            // draw result
            result.resize(new_w, new_h);
            for(unsigned x = 0; x < width(); x++){
                for(unsigned y = 0; y < height(); y++){
                    HSLAPixel* in_pixel = getPixel(x, y);
                    HSLAPixel* out_pixel = result.getPixel(width() - x - 1, height() - y - 1);
            ...
            for(unsigned x = 0; x < new_w; x++){
                for(unsigned y = 0; y < new_h; y++){
                    HSLAPixel* in_pixel = getPixel(x, y);
                    HSLAPixel* out_pixel = result.getPixel(x, y);
                    in_pixel->a = out_pixel->a;
                    in_pixel->h = out_pixel->h;
                    in_pixel->l = out_pixel->l;
                    in_pixel->s = out_pixel->s;
                }
            }
            return;
        }
        if(degree == 90){
            unsigned new_h = width();
            unsigned new_w = height();
            Image result;
            // draw result
            result.resize(new_w, new_h);
            for(unsigned x = 0; x < width(); x++){
                for(unsigned y = 0; y < height(); y++){
                    HSLAPixel* in_pixel = getPixel(x, y);
                    HSLAPixel* out_pixel = result.getPixel(height() - y - 1, x);
                    out_pixel->a = in_pixel->a;
                    out_pixel->h = in_pixel->h;
                    out_pixel->l = in_pixel->l;
                    out_pixel->s = in_pixel->s;
        ...
        if(degree == 270){
            ...
            for(unsigned x = 0; x < width(); x++){
                for(unsigned y = 0; y < height(); y++){
                    HSLAPixel* in_pixel = getPixel(x, y);
                    HSLAPixel* out_pixel = result.getPixel(y, width() - x - 1);
                    out_pixel->a = in_pixel->a;
                    out_pixel->h = in_pixel->h;
                    out_pixel->l = in_pixel->l;
                    out_pixel->s = in_pixel->s;
