...
class BinaryTree
{
  ...
  private:
     ...
     * @param subRoot The current node in the recursion
     * @param base the base value to be compared
     * @param head check if the head element
     */
    bool isOrdered(const Node* subRoot, T &base, bool &head) const;

    /**
     * Private helper function for the public printPaths function.
     * @param subRoot The current node in the recursion
     * @param paths vector of vectors that contains path of nodes
     * @param one_path vector that contains one path of node
     */
    void printPaths(const Node* subRoot, vector<vector<T>> &paths, vector<T> &one_path) const;
     ...
     */
    void sumDistances(const Node* subRoot, int &sum, int &depth) const;
