                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    TreeNode* smallest = singleQueue.front();
    if(mergeQueue.front()->freq < singleQueue.front()->freq) smallest = mergeQueue.front();
    if(smallest == singleQueue.front()){
        singleQueue.pop();
    } else {
        mergeQueue.pop();
    }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(singleQueue.size() + mergeQueue.size() > 1){
        ...
        TreeNode* internal_node = new TreeNode((child1->freq).getFrequency() + (child2->freq).getFrequency());
        internal_node->left = child1;
        internal_node->right = child2;
        mergeQueue.push(internal_node);
    }

    // assign the root
    if(singleQueue.size() != 0) root_ = singleQueue.front();
    if(mergeQueue.size() != 0) root_ = mergeQueue.front();
}
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.getNextBit() == 1){
        Frequency data(bfile.getNextByte(), 0);
