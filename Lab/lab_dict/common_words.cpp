...
void CommonWords::init_common()
{
    ...
    for (size_t i = 0; i < file_word_maps.size(); i++) {
        for(std::pair<const string, unsigned int>& key_value: file_word_maps[i]){
            if(common.find(key_value.first) == common.end()){
                common[key_value.first] = 0;
            }
            common[key_value.first]++;
        }
    }
}
...
vector<string> CommonWords::get_common_words(unsigned int n) const
{
    ...
    for(const std::pair<const string, unsigned int>& word : common){
        if(word.second == file_word_maps.size()){
            // go through all files to check its appearence
            size_t i;
            ...
            if(i == file_word_maps.size()){
                // prevent multiple include
                vector<string>::iterator it;
                for(it = out.begin(); it != out.end(); it++){
                    if(*it == word.first) break;
                }
                if(it == out.end()) out.push_back(word.first);
            }
        }
    }
    return out;
