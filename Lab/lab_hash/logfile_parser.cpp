...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
        ...
        if(pageVisitedTable[ll.url] == false){
            uniqueURLs.push_back(ll.url);
            pageVisitedTable[ll.url] = true;
        }

        // the unique equals to the combination of customer and url
        string unique_key = ll.customer + ll.url;
        if(!hasVisited(ll.customer, ll.url) || dateVisited(ll.customer, ll.url) < ll.date){
            whenVisitedTable[unique_key] = ll.date;
        }
    }
    infile.close();
}
