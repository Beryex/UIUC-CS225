...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(first_larger_idx < subroot->elements.size() && subroot->elements[first_larger_idx] == key) return subroot->elements[first_larger_idx].value;
     ...
     */
    // if leaf, return default V
    if(subroot->is_leaf) return V();
    
    // else return recursive result
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    /* TODO Your code goes here! */
    DataPair target = child->elements[mid_elem_idx];
    // deal with parent's children
    parent->children.insert(child_itr, new_right);
    parent->children[child_idx]= new_left;
    // deal with parent's element
    parent->elements.insert(elem_itr, target);
    // deal with right and left children
