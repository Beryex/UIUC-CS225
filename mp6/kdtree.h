...
class KDTree
{
  ...
  private:
    ...
    void construct(KDTreeNode*& subtree, int d, vector<Point<Dim>> sort_vector);

    /** Helper function to find k-th smallest point in dimension d */
    Point<Dim> find_k_smallest(int d, vector<Point<Dim>>& sort_vector);

    /** function for assignment and copy constructor*/
    KDTreeNode* copy(const KDTreeNode* subtree);

    /** helper function for clear*/
    void clear(KDTreeNode* subtree);

    /** helper function for findNearestNeighbor*/
    void findNearestNeighbor(const KDTreeNode* subtree, Point<Dim>& currentBest, 
