...
TileImage::TileImage(const PNG& source, int _size) : image_(cropSourceImage(source)) {
    averageColor_ = calculateAverageColor();
    generateResizedImage(_size);
    image_ = PNG();
}
...
void TileImage::generateResizedImage(int resolution) {
...
void TileImage::paste(PNG& canvas, int startX, int startY, int resolution) {
    ...
    if (resized_.width() == 0) {
        generateResizedImage(resolution);
    }
